...
const difficultyLevelOptions = ['Beginner', 'Intermediate', 'Advance'];

const UpdateExercise: React.FC = () => {
  ...
  const onChangeFilesHandler = (e: any) => {
    ...
    if (isMultipleFiles) {
      ...
      const intoArray = [thumbnail];
      if (intoArray.length > 1) {
        toast.error('You can upload only 1 thumbnail');
      } else {
        setThumbnail(intoArray);
      }
    }
  };
  ...
  const createExercise = async () => {
    try {
      ...
      console.log('Failed while creating exercise:', error);
      toast.error('Failed while creating exercise');
    } finally {
      setLoading(false);
    }
  };
  ...
  const uploadThumbnail = async () => {
    ...
    thumbnail.map((thumbnail, _) => {
      thumbnailData.append('files', thumbnail);
    });

    try {
      let response = await axios({
        method: 'POST',
        url: `${baseUrl}/exercise/uploads/exercises`,
        data: thumbnailData,
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      console.log('Exercise Thumbnail: ', response);
      ...
      console.log('Failed while uploading thumbnail: ', error);
      toast.error('Failed while uploading thumbnail');
      setLoading(false);
    }
  };
  ...
  const uploadFiles = async () => {
    ...
    try {
      ...
      console.log('Exercise Files: ', response);
      if (response.status === 200) {
        setFormData((prevData) => ({
          ...prevData,
...
//   }, [formData.thumbnail]);
  ...
  useEffect(() => {
    ...
    if (preData) {
      console.log(preData);
      setFormData({
        exerciseName: preData.exerciseName,
        bodyEmphasis: preData.bodyEmphasis,
        difficultyLevel: preData.difficultyLevel,
  ...
  return (
    <>
      <Toaster />
      <Breadcrumb pageName="Update Exercise" />
